...
def obtener_servicios_por_habitacion(id_habitacion):
    try:
        ...
        if len(resultado) == 0:
            return jsonify(
                {"Error": "No hay servicios incluidos en esta habitacion"}
            ), 400
...
def ingresar_reserva():
    ...
    try:
        resultado = hoteles.comprobar_disponibilidad_habitacion(
            datos["id_habitacion"], datos["salida"], datos["llegada"]
        )
        if len(resultado) != 0:
            return jsonify(
                {"Error": "La habitacion no esta disponible en esas fechas"}
            ), 400
    ...
    return jsonify(
        {
            "Exito": f"Los datos se cargaron exitosamente, el id de la reserva es {resultado}"
        }
    ), 200
...
def actualizar_estado_reserva(id, cliente):
    ...
    return jsonify(
        {
            "Exito": f"El estado de la reserva paso a cancelado, asi como los servicio relacionados a esa reserva"
        }
    ), 200
